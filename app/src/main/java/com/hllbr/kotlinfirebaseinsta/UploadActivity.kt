...
class UploadActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        //operations in gallery

        if (requestCode == 2 && resultCode == Activity.RESULT_OK && data != null ) {
            selectedPicture = data.data// data.data have location of image
            try{
    ...
    fun uploadClicked(view : View){
        //UUID -> UNİVERSAL UNIC ID(uygurma bir sınıf)
        ...
        if (selectedPicture != null){
            imagesReference.putFile(selectedPicture!!).addOnSuccessListener { taskSnapshot->
                //Database - FireStore
                val uploadPicturesReference = FirebaseStorage.getInstance().reference.child("images").child(imageName)
                uploadPicturesReference.downloadUrl.addOnSuccessListener { uri ->
